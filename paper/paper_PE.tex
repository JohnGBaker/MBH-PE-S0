...
The space-based gravitational wave detector LISA will observe mergers of supermassive black hole binary systems (SMBHBs) to cosmological distances, as well as inspiralling stellar-origin (or stellar-mass) binaries (SBHBs) years before they enter the LIGO/Virgo band. Much remains to be explored for the parameter recovery of both class of systems. Previous SMBHB analyses relied on inspiral-only signals and/or a simplified Fisher matrix analysis, while SBHBs have not yet been analyzed with Bayesian methods.  We present the first simulations of Bayesian inference for the parameters of supermassive black hole systems including consistently the merger and ringdown of the signal, as well as higher harmonics. We use an instrumental response that includes finite-armlength effects at high frequencies, but we ignore the spin degrees of freedom and set the noise realization to zero. We find that restricting the merger-dominated signal to its dominant harmonic can make the extrinsic likelihood very degenerate. Including higher harmonics proves to be crucial to break degeneracies and considerably improves the localization of the source, with a surviving bimodality in the sky position. We explore the pre-merger parameter recovery by cutting the signals at different times before merger, and find a degenerate structure with eight modes in the sky reducing to two modes only shortly before merger. We analyze the respective roles of the time- and frequency-dependence of the LISA response in breaking degeneracies, and find that finite-armlength effects prove crucial in eliminating six of the eight modes in the sky, while the effect of the LISA motion is limited for our short-lived example signals. We also present simulations of Bayesian inference for the extrinsic parameters of SBHBs, and show that although unimodal, their posterior distributions can have non-Gaussian features.
...
\ee
with $H$ the transverse-traceless matrix representing the gravitational wave, $k$ the wave propagation vector, $L$ the delay along one arm, taken to be fixed, $n_{l}$ the link unit vectors (from $s$ to $r$) and $p_{s}$, $p_{r}$ the positions of the spacecrafts. Here $n_{l}$, $p_{s}$ and $p_{r}$ are evaluated at the same time $t$.
...
\end{align}
\esub
These functions are the familiar pattern functions of ground-based detectors (for $\psi_{L}=0$) \SM{[add citation]}.
...
Introducing the standard matched-filter inner \SM{[add reference]} product (also called overlap) as
...
\begin{table}
	\begin{tabular}{|c|c|c|c|}
		...
		\hline
		SBHB & $(2,2)$ & $500$ & $2$ ms \\
		SMBHB & $(2,2)$ & $300$ & $1.2$ ms \\
		SMBHB & $(2,2),(2,1),(3,3),(4,4),(5,5)$ & $300$ & $15$ ms \\
		\hline
	\end{tabular}
	\label{tab:likecost}
...
In this process we also impose the constrain that the step is never larger than $10^{-10}$ times the initial scaling.
...
In the case (iv), \textit{Frozen low-$f$}, the response is equivalent to two coplanar LIGO-type detectors, lying motionless at the same location and rotated with respect to each other by $\pi/4$, and there is no other information on the source's sky position other than the pattern functions of the two effective detectors. Contrarily to networks of ground-based observatories, we have no triangulation information from times of arrival at different detectors.
This is the most degenerate case, and will be useful to get an analytical understanding of the approximate degeneracies occuring when using the more complicated full response (i).

The effect of these approximations on the response is shown in Fig.~\ref{fig:responseapproxtdia22}. We display the strain transfer function $\calT_{h_{a}}^{22}(f)$ as defined in~\eqref{eq:defhaet} for the mode $h_{22}$, with the four lines showing the four approximations (i)-(iv). Vertical lines also indicate the times to merger highlighted in Sec.~\ref{subsec:MBHBacctime}, as well as the peak frequency. Paralleling this figure to Figs.~\ref{fig:hctdiahm} and~\ref{fig:modeoverlapsoftime}, we stress that most of the SNR is accumulated in the very last instants before merger and at the merger itself, due to the noise normalization not visible at the level of the transfer function.
...
The priors used are logarithmic in mass, flat in luminosity distance, and flat for the angles: flat on the sphere for the pairs of angles $(\iota, \varphi)$ and $(\lambda, \beta)$, and flat for the polarization $\psi$. We expect priors to be unimportant for the masses, since they are well determined in this very high SNR limit. The luminosity distance, as we will see, can be less well determined in the absence of higher harmonics, and one should keep in mind that the prior choice does affect the posterior there.
...
To illustrate the role of higher harmonics in the analysis, we will present two classes of results: ``22'', where we perform both the simulation and the inference by only including the dominant harmonic $h_{22}$, and ``HM'', where we perform both the simulation and the inference with all available harmonics $(h_{22}, h_{21}, h_{33}, h_{44}, h_{55})$.
We do not consider the case where the simulation includes all harmonics and the inference is restricted to the 22 mode, as this will most likely bias the inferred parameters by an amount larger than the statistical uncertainties.
In fact, the SNR calculation in Fig.~\ref{fig:modeoverlapsoftime} suggests that even the set of five modes included in this study may not be sufficient.
The question of how many harmonics are necessary to keep the systematic errors below the statistical uncertainties is important and will be investigated in the future.
...
In both Cases I and II, including the higher harmonics greatly reduces the uncertainties in the inferred parameters, consistently with previous studies~\cite{Arun+07a, TriasSintes07, PorterCornish08, McWilliams+09}.
The improvement is much larger than we would expect from the gain in total SNR and a scaling of statistical errors $\sim 1/\SNR$.
The largest effect is found for the angular parameters and luminosity distance.
The mass posteriors are slightly more precise, but remain qualitatively similar.
...
\ee
we see that simultaneously changing $\iota \rightarrow \pi-\iota$ and $\psi_{L} \rightarrow \pi - \psi_{L}$ leaves $F_{a}^{\ell m}$, $F_{e}^{\ell m}$ unchanged.
