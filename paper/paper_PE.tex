...
The inference problem amounts to computing a likelihood function, i.e.~the probability of observing the measured data given a model of the source and detector, and a particular parameter vector for the model.
In LISA, this problem is more challenging than in kilometer-scale ground-based interferometers.
Depending on the masses and spins of the black holes, the complexity arises from the much longer duration of the waveform, the much larger expected signal-to-noise ratios (SNRs), or the much more complex response of the LISA constellation to a given waveform.
The large number of cycles in the LISA band and the large SNR require very accurate modeling of the waveform, including the merger and ringdown regimes, the effects of spin and corrections of higher order than the quadrupole.
...
Furthermore, due to the LISA arm length, for many sources the long-wavelength approximation will not hold.
As we will see in this study, a proper treatment of these effects is necessary in order to break severe degeneracies between the source parameters.
...
The full posterior distributions are then compared with their Fisher approximations, widely used in gravitational-wave analysis.
